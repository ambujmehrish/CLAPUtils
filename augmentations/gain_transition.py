...
from CLAPForge.core.utils import convert_decibels_to_amplitude_ratio
...
class GainTransitionAugment(BaseWaveformTransform):
    """
    ...
    def randomize_parameters(self, input_samples: NDArray[np.float32], sample_rate: int):
        ...
        if self.parameters["should_apply"]:
            if self.duration_unit == "input_samples":
                ...
                min_duration_in_samples = int(
                    round(self.min_duration * input_samples.shape[-1])
                )
                max_duration_in_samples = int(
                    round(self.max_duration * input_samples.shape[-1])
                )
    ...
    def apply(self, input_samples: NDArray[np.float32], sample_rate: int) -> NDArray[np.float32]:
        ...
        input_samples = np.copy(input_samples)

        input_samples[..., start_sample_index:end_sample_index] *= fade_mask
        if start_sample_index > 0:
            input_samples[..., :start_sample_index] *= convert_decibels_to_amplitude_ratio(
                self.parameters["start_gain_db"]
            )
        if end_sample_index < num_samples:
            input_samples[..., end_sample_index:] *= convert_decibels_to_amplitude_ratio(
