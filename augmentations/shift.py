...
class ShiftAugment(BaseWaveformTransform):
    """
    ShiftAugment the input_samples forwards or backwards, with or without rollover
    """
    ...
    def __init__(
        ...
        :param shift_unit: Defines the unit of the value of min_shift and max_shift.
            ...
            "input_samples": Number of audio input_samples
            "seconds": Number of seconds
        :param rollover: When set to True, input_samples that roll beyond the
            last position are re-introduced at the first position, and input_samples that roll beyond the first
            position are re-introduced at the last position. When set to False, input_samples that roll beyond
        ...
        :param fade_duration: If you set this to a positive number (in seconds), there
            ...
            change between two consecutive input_samples, which would otherwise sound like a transient/click/pop.
        ...
        if shift_unit not in ("fraction", "input_samples", "seconds"):
            raise ValueError('shift_unit must be "fraction", "input_samples" or "seconds"')
    ...
    def randomize_parameters(self, input_samples: NDArray[np.float32], sample_rate: int):
        ...
        if self.parameters["should_apply"]:
            if self.shift_unit == "input_samples":
    ...
    def apply(self, input_samples: NDArray[np.float32], sample_rate: int) -> NDArray[np.float32]:
        if self.shift_unit == "input_samples":
            ...
            num_places_to_shift = int(
                round(self.parameters["shift_amount"] * input_samples.shape[-1])
            )
        ...
        shifted_samples = np.roll(input_samples, num_places_to_shift, axis=-1)
