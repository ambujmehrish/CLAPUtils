...
class AirAbsorptionAugment(BaseWaveformTransform):
    """
    ...
    attenuate according to the distance law. GainAugment augmentation needs to be done separately.
    ...
    def apply(
        ...
        if len(input_samples.shape) == 1:
            stft = librosa.stft(input_samples, n_fft=n_fft)

            # Compute mask
            mask = np.tile(linear_target_attenuations, (stft.shape[1], 1)).T

            # Compute target degraded audio
            result = librosa.istft(stft * mask, length=len(input_samples), dtype=np.float32)

        else:
            result = np.zeros_like(input_samples, dtype=np.float32)

            for chn_idx, channel in enumerate(input_samples):
