...
class ClipAugment(BaseWaveformTransform):
    """
    ClipAugment audio by specified values. e.g. set a_min=-1.0 and a_max=1.0 to ensure that no
    input_samples in the audio exceed that extent. This can be relevant for avoiding integer
    ...
    This transform is different from ClippingDistortionAugment in that it takes fixed values
    for clipping instead of clipping a random percentile of the input_samples. Arguably, this
    transform is not very useful for data augmentation. Instead, think of it as a very
    cheap and harsh limiter (for input_samples that exceed the allotted extent) that can
    ...
    def apply(self, input_samples: NDArray[np.float32], sample_rate: int) -> NDArray[np.float32]:
        return np.clip(input_samples, self.a_min, self.a_max)
