...
class RoomSimulateAugment(BaseWaveformTransform):
    """
    ...
    > augment = RoomSimulateAugment(use_ray_tracing=False)
    Quickly add reverberation without caring for accuracy.

    > augment = RoomSimulateAugment(calculation_mode="rt60", min_target_rt60=0.06, max_target_rt60=0.06, min_size_x = ...)
    Augment with randomly selected room impulse responses that have an RT60 of 0.06.

    > augment = RoomSimulateAugment(min_mic_distance=1.0, max_mic_distance=1.0)
    Augment with a RIR captured by all positions of the microphone on a sphere, centred around the source at 1m

    > augment = RoomSimulateAugment(min_mic_distance=1.0, max_mic_distance=1.0, min_mic_elevation=0.0, max_mic_elevation=0.0)
    ...
    def randomize_parameters(self, input_samples: NDArray[np.float32], sample_rate: int):
        ...
        try:
            ...
            print(
                ...
                "To install the optional pyroomacoustics dependency of CLAPForge,"
    ...
    def apply(self, input_samples: NDArray[np.float32], sample_rate: int) -> NDArray[np.float32]:
        ...
        # This is the same as ImpulseResponseAugment transform
        if input_samples.ndim > 1:
            signal_ir = []
            for i in range(input_samples.shape[0]):
                channel_conv = convolve(input_samples[i], rir)
                signal_ir.append(channel_conv)
            signal_ir = np.array(signal_ir, dtype=input_samples.dtype)
        else:
            signal_ir = convolve(input_samples, rir).astype(input_samples.dtype)
