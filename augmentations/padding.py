...
class PaddingAugment(BaseWaveformTransform):
    """
    ...
    def __init__(
        ...
        """
        :param mode: PaddingAugment mode. Must be one of "silence", "wrap", "reflect"
    ...
    def randomize_parameters(self, input_samples: NDArray[np.float32], sample_rate: int):
        ...
        if self.parameters["should_apply"]:
            input_length = input_samples.shape[-1]
    ...
    def apply(self, input_samples: NDArray[np.float32], sample_rate: int) -> NDArray[np.float32]:
        ...
        untouched_length = input_samples.shape[-1] - padding_length

        if self.mode == "silence":
            input_samples = np.copy(input_samples)
            if self.pad_section == "start":
                input_samples[..., :padding_length] = 0.0
            else:
                input_samples[..., -padding_length:] = 0.0
        else:
            ...
            if self.pad_section == "start":
                input_samples = input_samples[..., -untouched_length:]
            else:
                input_samples = input_samples[..., :untouched_length]

            input_samples = np.pad(input_samples, pad_width, self.mode)
