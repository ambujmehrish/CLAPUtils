...
from CLAPForge.core.utils import a_weighting_frequency_envelope
from CLAPForge.core.utils import calculate_desired_noise_rms, calculate_rms
...
def generate_decaying_white_noise(
    ...
    # NormalizeAugment to unit energy
    fsig /= np.sqrt(np.mean(fsig**2))

    return fsig.astype(np.float32)


class ColorNoiseAugment(BaseWaveformTransform):
    """
    Adds noise to the input input_samples with a decaying frequency spectrum resulting in "color" noise.
    ...
    def randomize_parameters(self, input_samples: np.ndarray, sample_rate: int):
    ...
    def apply(
        ...
        if input_samples.ndim == 1:
            ...
            n_channels = input_samples.shape[0]

        noise_with_unit_rms = generate_decaying_white_noise(
            size=input_samples.shape,
        ...
        if n_channels > 1:
            return input_samples + noise_with_unit_rms * desired_noise_rms
        else:
            return input_samples + noise_with_unit_rms * desired_noise_rms
