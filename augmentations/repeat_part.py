...
from CLAPForge.core.utils import get_crossfade_mask_pair

# 0.00025 seconds corresponds to 2 input_samples at 8000 Hz
DURATION_EPSILON = 0.00025


class RepeatPartAugment(BaseWaveformTransform):
    """
    ...
    * Digital audio glitches, such as a buffer underrun in video games,
    ...
    def __init__(
        ...
        :param part_transform: An optional callable (CLAPForge transform) that
    ...
    def randomize_parameters(self, input_samples: NDArray[np.float32], sample_rate: int):
        ...
        if self.parameters["should_apply"]:
            ...
            if (
                ...
                >= input_samples.shape[-1] - self.parameters["part_num_samples"]
            ):
                warnings.warn(
                    "The input sound is not long enough for applying the RepeatPartAugment"
    ...
    def get_crossfade_length(self, sample_rate: int) -> int:
        ...
        if crossfade_length < 2:
            warnings.warn(
                ...
                " crossfade length of 2 input_samples."
            )
            crossfade_length = 2
    ...
    def apply(self, input_samples: NDArray[np.float32], sample_rate: int) -> NDArray[np.float32]:
        ...
        for i in range(self.parameters["repeats"]):
            ...
            if self.mode == "replace" and last_repetition_end_index > input_samples.shape[-1]:
                break

        result_length = input_samples.shape[-1]
        if self.mode == "insert":
            result_length += (
        ...
        if input_samples.ndim == 1:
            ...
            result_shape = (input_samples.shape[0], result_length)
        ...
        if self.crossfade:
            ...
            result_placeholder[..., : repeats_start_index - half_crossfade_length] = (
                input_samples[..., : repeats_start_index - half_crossfade_length]
            )
            result_placeholder[
                ...,
                repeats_start_index
            ...
            result_placeholder[..., :repeats_start_index] = input_samples[
                ..., :repeats_start_index
            ]
        ...
        if self.mode == "replace" and parts[-1]["end_idx"] > input_samples.shape[-1]:
            truncated_part_length = input_samples.shape[-1] - parts[-1]["start_idx"]
        ...
        if self.mode == "insert":
            ...
            result_placeholder[..., last_repetition_end_index:] = input_samples[
                ..., -(result_length - last_repetition_end_index) :
            ]
        else:
            if self.crossfade:
                last_crossfade_start_idx = last_repetition_end_index - crossfade_length
                if last_crossfade_start_idx < input_samples.shape[-1]:
                    last_crossfade_length = crossfade_length
                    if last_repetition_end_index > input_samples.shape[-1]:
                        # Truncate crossfade
                        last_crossfade_length = (
                            input_samples.shape[-1] - last_crossfade_start_idx
                        )
