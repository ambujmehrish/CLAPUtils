...
from CLAPForge.core.utils import (
...
class ShortNoisesAugment(BaseWaveformTransform):
    ...
    def __init__(
        ...
        :param signal_gain_db_during_noise: GainAugment applied to the signal during a short noise.
        ...
        self._load_sound = functools.lru_cache(maxsize=lru_cache_size)(
            ShortNoisesAugment.__load_sound
        )
    ...
    def randomize_parameters(self, input_samples: NDArray[np.float32], sample_rate: int):
        ...
        if self.parameters["should_apply"]:
            input_sound_duration = len(input_samples) / sample_rate
    ...
    def apply(self, input_samples: NDArray[np.float32], sample_rate: int) -> NDArray[np.float32]:
        ...
        noise_placeholder = np.zeros_like(input_samples)
        ...
        for sound_params in self.parameters["sounds"]:
            ...
            if self.noise_rms == "relative_to_whole_input":
                clean_rms = calculate_rms(input_samples)
            else:
                clean_rms = calculate_rms(input_samples[start_sample_index:end_sample_index])

            if noise_samples.shape[-1] > 0:
                # GainAugment here describes just the gain from the fade in and fade out.
        ...
        if gain_signal:
            # GainAugment the original signal before mixing in the noises
            signal_mask *= self.signal_gain_db_during_noise
            signal_mask = convert_decibels_to_amplitude_ratio(signal_mask)
            return input_samples * signal_mask + noise_placeholder
        else:
            # Return a mix of the input sound and the added sounds
            return input_samples + noise_placeholder

    def __getstate__(self):
        state = self.__dict__.copy()
        warnings.warn(
            "Warning: the LRU cache of ShortNoisesAugment gets discarded when pickling it."
            " E.g. this means the cache will not be used when using ShortNoisesAugment"
