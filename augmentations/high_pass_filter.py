from CLAPForge.augmentations.base_butterword_filter import BaseButterworthFilter


class HighPassFilterAugment(BaseButterworthFilter):
    """
    ...
    def __init__(
        ...
        if min_cutoff_freq <= 0:
            raise ValueError(
                f"HighPassFilterAugment requires min_cutoff_freq > 0. Got {min_cutoff_freq}."
            )
        super().__init__(
